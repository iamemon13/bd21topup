...
export default function AdminUsersPage() {
  ...
  async function loadUsers() {
    try {
      ...
      // 🔒 FIXED: Fetching current user role via secure API instead of direct DB query
        ...
        <div className="rounded-2xl border border-cyan-400/20 bg-[#0b2545] p-5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">Manage users, roles, and wallets</p>
            </div>
          ...
          {!loading && (
            <p className="mt-2 text-[10px] text-slate-500">Showing {filteredUsers.length} of {users.length} users</p>
          )}
        </div>

        <div className="mt-5 space-y-4">
          {loading && <div className="rounded-2xl border border-cyan-400/20 bg-[#0b2545] p-5 text-center text-sm text-slate-400">Loading users...</div>}
          
          {!loading && users.length > 0 && filteredUsers.length === 0 && (
            <div className="rounded-2xl border border-white/10 bg-[#0b2545] p-8 text-center">
              <div className="text-3xl">🔍</div>
              <p className="mt-3 text-sm font-bold text-slate-300">No matching users found</p>
            </div>
          )}

          {filteredUsers.map((user) => (
            <div key={user.id} className="rounded-2xl border border-cyan-400/20 bg-[#0b2545] p-5 flex flex-col sm:flex-row gap-4 justify-between">
              
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h2 className="truncate text-xl font-bold">{user.full_name || "Unknown"}</h2>
                  {user.role && (
                    <span className={`shrink-0 rounded-full border px-2 py-1 text-[9px] font-black uppercase ${
                      user.role === "super_admin" ? "border-amber-400/40 bg-amber-400/10 text-amber-300" :
                      user.role === "admin" ? "border-cyan-400/20 bg-cyan-400/10 text-cyan-300" :
                      user.role === "editor" ? "border-purple-400/20 bg-purple-400/10 text-purple-300" :
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-[10px] font-mono text-slate-500 bg-[#07182f] px-2 py-1 rounded">ID: {user.id}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:items-end gap-3 min-w-[200px]">
                <div className="rounded-xl border border-cyan-400/10 bg-[#07182f] p-3 text-center sm:text-right w-full">
                  <p className="text-[10px] font-bold uppercase text-slate-500">Wallet Balance</p>
                ...
                <div className="flex gap-2 w-full">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setAmount(""); setNote(""); setAction("add");
                    }}
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-purple-400/30 bg-[#0b2545] p-5 shadow-2xl">
            <div className="flex justify-between mb-4">
              <h2 className="text-xl font-bold text-purple-400">Manage Role & Permissions</h2>
              <button onClick={() => setSelectedRoleUser(null)} className="rounded-lg bg-slate-700 px-3 py-1 text-sm font-bold text-white hover:bg-slate-600">✕</button>
            </div>
            <p className="text-sm text-slate-300 mb-4">{selectedRoleUser.full_name || selectedRoleUser.email}</p>

            <div className="mb-4">
              <label className="block text-[10px] font-bold uppercase text-slate-500 mb-2">User Role</label>
              <select 
                value={editRole} 
            ...
            {(editRole === "admin" || editRole === "editor") && (
              <div className="mb-4">
                <label className="block text-[10px] font-bold uppercase text-slate-500 mb-2">Specific Permissions</label>
                <div className="space-y-2 rounded-xl bg-[#07182f] p-3 border border-white/10">
                  {AVAILABLE_PERMISSIONS.map(perm => (
                    <label key={perm.id} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-white/5 rounded-lg transition">
                      <input 
                        type="checkbox" 
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-cyan-400/30 bg-[#0b2545] p-5 shadow-2xl">
            <div className="flex justify-between mb-2">
              ...
              <button onClick={closeWalletModal} className="rounded-lg bg-slate-700 px-3 py-1 text-sm font-bold">✕</button>
            </div>
            <p className="text-sm text-slate-300 mb-4">{selectedUser.full_name}</p>
            ...
            <div className="rounded-xl bg-[#07182f] p-4 text-center border border-white/5">
              ...
              <p className="text-2xl font-black text-cyan-400 mt-1">৳{Number(selectedUser.wallet_balance ?? 0).toLocaleString("en-BD")}</p>
            </div>

            <input type="number" min="0" step="0.01" placeholder="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-4 w-full rounded-xl border border-white/10 bg-[#07182f] p-3 text-white outline-none focus:border-cyan-400/50" />

            <div className="mt-3 flex gap-3">
              <button type="button" onClick={() => setAction("add")} className={`flex-1 rounded-xl py-3 font-black ${action === "add" ? "bg-cyan-400 text-black" : "bg-gray-700 text-white"}`}>Add</button>
              <button type="button" onClick={() => setAction("remove")} className={`flex-1 rounded-xl py-3 font-black ${action === "remove" ? "bg-red-500 text-white" : "bg-gray-700 text-white"}`}>Remove</button>
            </div>

            <textarea placeholder="Admin note" value={note} onChange={(e) => setNote(e.target.value)} className="mt-3 h-20 w-full resize-none rounded-xl border border-white/10 bg-[#07182f] p-3 text-sm text-white outline-none placeholder:text-slate-600 focus:border-cyan-400/50" />

            <button type="button" onClick={updateWallet} className={`mt-4 w-full rounded-xl py-3 font-black ${action === "remove" ? "bg-red-500 text-white" : "bg-cyan-400 text-black"}`}>
